...
topTenErrURLs = (access_logs.filter(lambda log : log.response_code != 200).map(lambda log: (log.endpoint, 1))
                  .reduceByKey(lambda a, b : a + b)).takeOrdered(10, lambda s: -s[1] )
...
uniqueHostCount = access_logs.map(lambda log: log.host).distinct().count()
...
# first a set(unique items) is created from list of hosts and then total of them are calculated
dayHostCount = dayGroupedHosts.map(lambda dayHost : (dayHost[0], len(set(dayHost[1]))))

dailyHosts = (dayHostCount
              .map(lambda h : (h[0], h[1]))).sortByKey().cache()
...
#no of hosts/ unique hosts (set)
avgDailyReqPerHost = (sortedByDay
                      .map(lambda h : (h[0], len(h[1])/len(set(h[1]))))).cache()
...
badRecords = (access_logs.filter(lambda log : log.response_code == 404)).persist() 
...
print '404 Errors by day: %s' % errDateSorted
